...
DEFAULT_LABEL_NAMES = ["Walgreens Receipts"]
DEFAULT_QUERY = ""
CACHE_VERSION = "walgreens-v1"
SPLIT_PARENT_MEMO = ""
...
def get_gmail_label_ids(service, label_names: list[str]) -> list[str]:
    if not label_names:
        return []

    response = service.users().labels().list(userId="me").execute()
    labels = response.get("labels", []) or []
    wanted = {name.strip().lower() for name in label_names if name and name.strip()}
    out = []
    for label in labels:
...
def parse_order_and_amounts(text: str):
    order_match = (
        re.search(r"\b\d{3}-\d{7}-\d{7}\b", text)
        or re.search(r"\bOrder\s*#\s*([0-9]{9,})\b", text, flags=re.IGNORECASE)
        or re.search(r"\bTransaction\s+ID:\s*([0-9]{3,})\b", text, flags=re.IGNORECASE)
    )
    order_id = order_match.group(0) if order_match else None

    amounts: list[str] = []
    patterns = [
        r"(Order\s+Summary|Order\s+Total|Total\s+before\s+tax|Total\s+for\s+this\s+order|Grand\s+Total|Total|Subtotal|Sales\s+tax|Price)\s*[:\-]?\s*\$?\s*([0-9][0-9,]*(?:\.[0-9]{2})?)",
        r"(?:charged|charge|order of|payment|amex|visa|mastercard|discover)\s*[:\-]?\s*\$?\s*([0-9][0-9,]*(?:\.[0-9]{2})?)",
        r"(Order\s+Total|Total\s+for\s+this\s+order|Grand\s+Total|Total)\s*[:\-]?\s*\$?\s*([0-9]+(?:\.[0-9]{2})?)",
        r"\$\s*([0-9]+(?:\.[0-9]{2})?)\s*(?:USD)?\s*(?:Order\s+Total|Total|Subtotal)",
    ]
...
def guess_category(subject: str, body: str) -> str:
    hay = f"{subject}\n{body}".lower()
    if any(k in hay for k in ("grocery", "food", "kitchen", "cocoa", "rice", "snack", "drink")):
        return "Groceries"
    if any(k in hay for k in ("pet", "dog", "cat")):
        return "Zoe"
    if any(k in hay for k in ("vitamin", "supplement", "medicine", "cerave", "lotion", "foundation", "skin", "itch")):
        return "Health/Skin Care"
    return "Shopping"
...
def fetch_walgreens_candidates(service, query: str, max_results: int, label_ids: list[str]):
    messages = []
    page_token = None
...
def search_candidate_for_transaction(
    ...
    queries = [
        f'from:Walgreens@ecs.walgreens.com "{target_amount_text}" after:{after_dt} before:{before_dt}',
        f'subject:("confirmed" OR "receipt" OR "shipped" OR "shopping" OR "pickup") "{target_amount_text}" after:{after_dt} before:{before_dt}',
        f'"Order total" "{target_amount_text}" after:{after_dt} before:{before_dt}',
        f'"Total: ${target_amount_text}" after:{after_dt} before:{before_dt}',
        f'from:Walgreens@ecs.walgreens.com "{target_amount_short}" after:{after_dt} before:{before_dt}',
        f'from:Walgreens@ecs.walgreens.com after:{after_dt} before:{before_dt}',
...
def ai_summarize_candidate(candidate: dict, api_key: str, model: str, charge_amount: str  = None):
    ...
    prompt = (
        "You classify Walgreens purchases for personal budgeting. "
    ...
    memo = normalize_summary((parsed.get("memo") or "Walgreens purchase").strip()[:180])
...
def simplify_item_name(name: str) -> str:
    ...
    n = re.sub(r"(?i)^(walgreens|nice!|cerave|the ordinary|wet n wild|physicians formula)\s+", "", n).strip()
    ...
    replacements = [
        ...
        (r"(?i)\bswiss miss\b", "hot cocoa mix"),
        (r"(?i)\bcream of rice\b", "cream of rice"),
        (r"(?i)\bfoundation pump\b", "foundation pump"),
    ]
...
def pick_items_for_charge(items: list[dict], tx_amount_milli: int, tolerance_milli: int = 250) -> list[dict]:
    # Keep only the item subset that best matches this charge amount.
...
def force_single_split_for_all_strategy(
    ...
    first_name = (items[0].get("name") if items else None) or "Walgreens Item"
...
def filter_transactions_by_payee(
    ...
    term = (contains_term or "walgreens").strip().lower()
    filtered = []
...
def parse_args():
    parser = argparse.ArgumentParser(description="Sync Walgreens email receipt info into YNAB transaction memos.")
    ...
    parser.add_argument(
        ...
        help="gaps = target likely incomplete records; all = reprocess all matching Walgreens records.",
    )
    parser.add_argument(
        "--depth",
        choices=["normal", "deep"],
    ...
    parser.add_argument(
        "--query",
        default=env_first("WALGREENS_GMAIL_QUERY", "walgreens_gmail_query", "GMAIL_QUERY", "gmail_query", default=DEFAULT_QUERY),
        help="Gmail search query.",
    )
    parser.add_argument(
        "--label",
        default=env_first("GMAIL_LABEL", "gmail_label", default=DEFAULT_LABEL_NAMES[0]),
        help="Gmail label name to constrain Walgreens receipt search.",
    )
...
def main():
    ...
    target_payee = env_first(
        "WALGREENS_PAYEE_NAME_TO_BE_PROCESSED",
        "walgreens_payee_name_to_be_processed",
        "YNAB_PAYEE_NAME_TO_BE_PROCESSED",
        "ynab_payee_name_to_be_processed",
    )
    payee_filter_mode = env_first(
        "WALGREENS_PAYEE_FILTER_MODE",
        "walgreens_payee_filter_mode",
        "YNAB_PAYEE_FILTER_MODE",
        "ynab_payee_filter_mode",
        default="contains",
    )
    payee_contains = env_first(
        "WALGREENS_PAYEE_CONTAINS",
        "walgreens_payee_contains",
        "YNAB_PAYEE_CONTAINS",
        "ynab_payee_contains",
        default="walgreens",
    )
    exclude_prime = env_bool("YNAB_EXCLUDE_PRIME", "ynab_exclude_prime", default=False)
    completed_payee = env_first(
        "WALGREENS_PAYEE_NAME_PROCESSING_COMPLETED",
        "walgreens_payee_name_processing_completed",
        default="Walgreens",
    )
    ...
    use_ai = env_bool("USE_AI_SUMMARIZATION", "use_ai_summarization", default=bool(openai_key))
    openai_model = env_first("OPENAI_MODEL", "openai_model", default="gpt-4o-mini")
    ai_cache_path = Path(env_first("AI_CACHE_FILE", "ai_cache_file", default="logs/walgreens_ai_cache.json"))
    ...
    label_ids = get_gmail_label_ids(gmail, [args.label])
    if label_ids:
        print(f"Gmail label: {args.label} -> {', '.join(label_ids)}")
    else:
        print(f"WARN Gmail label not found: {args.label}. Falling back to query-only search.")
    candidates = fetch_walgreens_candidates(gmail, effective_query, effective_max_results, label_ids)
